...
public class ManagerUpdateMovieHandler implements CommandHandler {
	...
	public String process(HttpServletRequest req, HttpServletResponse res) throws Exception {
		if (req.getMethod().equalsIgnoreCase("get")) {
			...
			try {
				int maxSize = 1024 * 1024 * 100;// 10M
				int mNo = Integer.parseInt(req.getParameter("mNo"));
				String uploadPath = service.selectFilePath(mNo); 

				System.out.println(uploadPath);

				int delState =Integer.parseInt(req.getParameter("yes"));
				System.out.println("삭제 상태는 " + delState);
				if (delState == 1) {
				...
				if (delState == 1) {
					movie = new Movie(mNo, name, poster, playTime, rating, openDate, closeDate, genre, steelCut, video,
							synopsis, actor, director, uploadPath);
					service.updateWithFile(movie);
					System.out.println("새로운 파일이 있으므로 파일도 함께 업데이트 완료하였습니다.");
					return "managerReadMovie.do?no=" + mNo;

				} else if (delState != 1) {
					movie = new Movie(mNo, name, playTime, rating, openDate, closeDate, genre, synopsis, actor,
							director);
